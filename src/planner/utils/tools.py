...
from planner.utils.rich import enforce_tags, print_config_tree
...
def join_string_underscore(
    texts: Optional[Union[str, Iterable[str]]] = None,
) -> Optional[str]:
    """Join strings with underscore.

    Args:
        texts (Optional[Union[str, Iterable[str]]], optional): The string or
            list of strings to be joined. Defaults to ``None``.

    Returns:
        Optional[str]: The joined string with underscore if `texts` is a list
            of strings, otherwise the input string or ``None``.
    """
    if texts is None:
        return None
